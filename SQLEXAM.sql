...
SELECT *
FROM naep
LIMIT 10 --by selecting all and limiting to 10, one can easily see what is populating this table

--2. Write a query that returns the first 50 records of the naep table.
SELECT *
FROM naep
LIMIT 50 --in this case we want a specified number of rows and not just a blanket, 10. 
...
SELECT state, COUNT(avg_math_4_score), AVG(avg_math_4_score), MIN(avg_math_4_score),MAX(avg_math_4_score) 
FROM naep
WHERE MAX(avg_math_4_score) - MIN(avg_math_4_score) > 30
GROUP BY state
...
SELECT state, ROUND(avg_math_4_score)
FROM naep
WHERE year = 2000
...
SELECT state, ROUND(AVG(avg_math_4_score))
FROM naep
WHERE year = 2000 AND avg_math_4_score < AVG(avg_math_4_score)
GROUP BY state
...
SELECT c.customerid, 
       ...
       orderid
FROM naep
LEFT JOIN  ON naep.finance = c.customerid
